...
course-demo/
  ...
  06_delta_end_to_end/    topics 3+4+5 combined
```
...
## Combined Delta notebook (topics 3+4+5)

`course-demo/06_delta_end_to_end/01_delta_lifecycle_combined.sql` — merges topics
3, 4 and 5 into one narrative using **one catalog `demo_delta_e2e`, one schema
`sales`, one table `orders`** instead of three sets. Each command appears once.
The three standalone notebooks are **unchanged** — use those for single topics.

**Deliberate ordering:** schema evolution (Part 5) comes *after* recovery (Part 4),
so restoring to an early version never conflicts with a later column. Do not
reorder these.

**Two conflicts that only appeared when combining — do not "fix" them back:**

1. **`GENERATED BY DEFAULT AS IDENTITY`, not `ALWAYS`.** `ALWAYS` forbids explicit
   inserts, so merge-from-a-clone recovery fails with
   `DELTA_IDENTITY_COLUMNS_EXPLICIT_INSERT_NOT_SUPPORTED`. Topic 3 uses `ALWAYS`
   and topic 5's table had no identity column, so neither hit it alone. Rule:
   **a table that must be recoverable needs `BY DEFAULT`.** This is now a taught
   point in Part 1.
2. **A clone starts its own history at version 0** — the source's versions are not
   carried over, so `RESTORE ... TO VERSION AS OF 1` on a fresh clone fails with
   `DELTA_CANNOT_RESTORE_TABLE_VERSION - Available versions: [0, 0]`. The rehearsal
   step now simulates the incident on the clone first, making version 0 the
   known-good point. Also a taught point.
